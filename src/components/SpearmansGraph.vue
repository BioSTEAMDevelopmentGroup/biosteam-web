  ...
  methods: {
    ...
    exportData() {
      if (this.boxplot !== null) {
        let rows = []
        let first_row_flag = true
        let first_row = [""]
        for (let key in this.spearman) {
          let row = []
          row.push("\"" + key + "\"")
          for (let idx in this.spearman[key]) {
            if (first_row_flag) {
              first_row.push(idx)
            }
            row.push(this.spearman[key][idx])
          }
          if (first_row_flag) {
            rows.push(first_row)
          }
        ...
        link.setAttribute("download", "spearman_results.csv");
